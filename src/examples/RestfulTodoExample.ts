...
} from "./models/RestfulTodoSchema"; // Adjust path
...
async function runRestfulTodoExample() {
  ...
  todoViewModel.todos$.subscribe((todos) => {
    console.log(
      ...
      todos
        ? todos
            .map(
              (t) => `[${t.isCompleted ? "x" : " "}] ${t.text} (ID: ${t.id})`
            )
            .join("\n")
        : "No todos"
    );
  ...
  try {
    ...
    if (!currentTodos || currentTodos.length === 0) {
      console.log(
        "No todos after fetch, cannot proceed with other operations as easily."
      );
    ...
    const addedTodo = await todoViewModel.addTodoCommand.execute(
      newTodoPayload
    );
    ...
    const todoToToggleId =
      currentTodos && currentTodos.length > 1 ? currentTodos[1].id : null;
    if (todoToToggleId) {
      console.log(
        `\n--- Toggling Completion for Todo (ID: ${todoToToggleId}) ---`
      );
      const todoBeforeToggle = currentTodos?.find(
        (t) => t.id === todoToToggleId
      );
      console.log(
        `Before toggle: ${todoBeforeToggle?.text} - Completed: ${todoBeforeToggle?.isCompleted}`
      );
      await todoViewModel.toggleTodoCompletion(todoToToggleId);
      // Re-fetch or check local data to see the change
      const todosAfterToggle = await todoViewModel.todos$
        .pipe(take(1))
        .toPromise();
      const todoAfterToggle = todosAfterToggle?.find(
        (t) => t.id === todoToToggleId
      );
      console.log(
        `After toggle: ${todoAfterToggle?.text} - Completed: ${todoAfterToggle?.isCompleted}`
      );
    } else {
    ...
    // Select an item
    const todoToSelectId =
      currentTodos && currentTodos.length > 0 ? currentTodos[0].id : null;
    if (todoToSelectId) {
