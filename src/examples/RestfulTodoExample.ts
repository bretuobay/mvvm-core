...
async function runRestfulTodoExample() {
  ...
  const initialTodos: RestfulTodoData[] = [
    { id: '1', text: 'Learn about RestfulApiModel', isCompleted: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    { id: '2', text: 'Implement Fake API', isCompleted: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    { id: '3', text: 'Create RestfulTodoViewModel', isCompleted: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
  ];
  ...
  todoViewModel.todos$.subscribe(todos => {
    console.log('Todos Updated:', todos ? todos.map(t => `[${t.isCompleted ? 'x' : ' '}] ${t.text} (ID: ${t.id})`).join('\n') : 'No todos');
  });
  ...
  try {
    ...
    const todoToUpdateId = addedTodo ? addedTodo.id : (currentTodos && currentTodos.length > 0 ? currentTodos[0].id : null);
    if (todoToUpdateId) {
    ...
    // Delete a todo
    const todoToDeleteId = addedTodo ? addedTodo.id : (currentTodos && currentTodos.length > 0 ? currentTodos[currentTodos.length -1].id : null);
    if (todoToDeleteId) {
