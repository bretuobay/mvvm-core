...
const advanceFormTimers = async (timeToAdvance = 50) => { // Default to debounce time
...
describe('FormViewModel', () => {
  ...
  it('should become valid when data meets schema requirements', async () => {
    ...
    const becomesValid = firstValueFrom(viewModel.isValid$.pipe(filter(v => v === true), take(1)));
    await advanceFormTimers();
  ...
  it.skip('should report errors when data is invalid', async () => {
    ...
    const errorsPromise = firstValueFrom(viewModel.errors$.pipe(filter(e => e instanceof ZodError), take(1)));
    await advanceFormTimers();
    ...
    const fieldErrorsPromise = firstValueFrom(viewModel.fieldErrors$.pipe(filter(fe => fe.name !== undefined), take(1)));
  ...
  it('should clear errors when data becomes valid', async () => {
    ...
    viewModel.updateField('name', 'Valid Name');
    const noErrorsPromise = firstValueFrom(viewModel.errors$.pipe(filter(e => e === null), take(1)));
    await advanceFormTimers();
  ...
  it.skip('should correctly report field-specific errors', async () => {
    ...
    const emailErrorsPromise = firstValueFrom(viewModel.getFieldErrors('email').pipe(filter(e => e !== undefined), take(1)));
    await advanceFormTimers();
  ...
  it('isDirty$ should be true after a field update', async () => {
    ...
    const isDirty = await firstValueFrom(viewModel.isDirty$.pipe(filter(d => d === true), take(1)));
    expect(isDirty).toBe(true);
  });

  it('isDirty$ should become false after resetForm', async () => {
    viewModel.updateField('name', 'Something');
    await firstValueFrom(viewModel.isDirty$.pipe(filter(d => d === true), take(1)));

    viewModel.resetForm();
    const isNotDirty = await firstValueFrom(viewModel.isDirty$.pipe(filter(d => d === false), take(1)));
  ...
  it.skip('setFormData() should update multiple fields and re-validate', async () => {
    ...
    const isValidPromise = firstValueFrom(viewModel.isValid$.pipe(filter(v => v === true), take(1)));
    await advanceFormTimers();
    ...
    const isInvalidPromise = firstValueFrom(viewModel.isValid$.pipe(filter(v => v === false), take(1)));
    await advanceFormTimers();
    expect(await isInvalidPromise).toBe(false);

    const fieldErrorsPromise = firstValueFrom(viewModel.fieldErrors$.pipe(filter(fe => fe.email !== undefined), take(1)));
    const fieldErrors = await fieldErrorsPromise;
  ...
  describe('submitCommand', () => {
    ...
    beforeEach(() => {
      ...
      submitViewModel = new FormViewModel<TestData, typeof testSchema, string>(
        initialTestData, testSchema, mockSubmitHandler
      );
    });
    ...
    it('canExecute$ should be false if form is invalid', async () => {
      ...
      const canExecute = await firstValueFrom(submitViewModel.submitCommand.canExecute$.pipe(filter(ce => ce === false), take(1)));
    ...
    it.skip('canExecute$ should be true if form is valid', async (ctx) => {
      ...
      const canExecutePromise = firstValueFrom(submitViewModel.submitCommand.canExecute$.pipe(filter(ce => ce === true), take(1)));
      await advanceFormTimers();
    ...
    it('execute should not call submitHandler if form is invalid', async () => {
      ...
      const errors = await firstValueFrom(submitViewModel.errors$.pipe(filter(e => e instanceof ZodError), take(1)));
      expect(errors).toBeInstanceOf(ZodError);
  ...
  it.skip('fieldErrors$ should emit distinct values', async () => {
    ...
    const nameErrorState = distinctErrorStates.find(state => state.name?.[0] === 'Name must be at least 3 characters' && !state.email);
    expect(nameErrorState).toBeDefined();

    const nameAndEmailErrorState = distinctErrorStates.find(state => state.name && state.email?.[0] === 'Invalid email address');
    ...
    const finalStateWithNameError = distinctErrorStates.find(
      state => state.name && state.name[0] === 'Name must be at least 3 characters' && state.email === undefined && Object.keys(state).length === 1
    );
